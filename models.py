...
class Empreendimento(db.Model):
    ...
    previsao_entrega = db.Column(db.String(255), nullable=True)
    valor_medio_unidades = db.Column(db.String(255), nullable=True)
    valor_a_partir_de = db.Column(db.String(255), nullable=True)
    tamanho_terreno = db.Column(db.String(255), nullable=True)
    ...
    arquiteto = db.Column(db.String(255), nullable=True)
    paisagismo = db.Column(db.String(255), nullable=True)
    decoracao = db.Column(db.String(255), nullable=True)
    tipologias = db.relationship('Tipologia', back_populates='empreendimento', lazy=True, cascade="all, delete-orphan")
...
class ExemploIA(db.Model):
    ...
    instrucao_correcao = db.Column(db.Text, nullable=True)
    data_criacao = db.Column(db.DateTime, default=datetime.utcnow)

class Tipologia(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    metragem = db.Column(db.String(100), nullable=False)
    suites = db.Column(db.String(100), nullable=True)
    vagas = db.Column(db.String(100), nullable=True)
    empreendimento_id = db.Column(db.Integer, db.ForeignKey('empreendimento.id'), nullable=False)
    empreendimento = db.relationship('Empreendimento', back_populates='tipologias')